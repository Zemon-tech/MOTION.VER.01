...
import {
  emitSubpageMetaToAncestors,
  emitSubpageDeletedToAncestors,
} from "../services/socketHub";
...
const PAGE_CREATE_LIMIT = 3;
const PAGE_CREATE_WINDOW_MS = 60_000;

function toPageCachePayload(page: any) {
  ...
  return {
    ...
    coverPosition: p.coverPosition ?? 50,
    isPublic: !!p.isPublic,
    shareToken: p.shareToken,
    linkEditEnabled: !!p.linkEditEnabled,
    linkExpiresAt: p.linkExpiresAt ?? null,
    sharedWith: Array.isArray(p.sharedWith) ? p.sharedWith : [],
    collaborators: Array.isArray(p.collaborators) ? p.collaborators : [],
    favoritedBy: Array.isArray(p.favoritedBy) ? p.favoritedBy : [],
    tags: Array.isArray(p.tags) ? p.tags : [],
    parentId: p.parentId ?? null,
    ancestors: Array.isArray(p.ancestors) ? p.ancestors : [],
    locked: !!p.locked,
...
async function cachePageRecord(page: any) {
  const payload = toPageCachePayload(page);
  if (!payload) return null;
  await RedisService.cachePage(payload);
  return payload;
}

async function invalidatePageRecord(pageId: string, slug?: string | null) {
  await RedisService.invalidatePageCache(pageId);
  if (slug) {
    await RedisService.invalidatePageSlug(slug);
  }
}

async function enforcePageCreateRateLimit(res: Response, userId: string) {
  const rateLimit = await RedisService.consumePageCreateQuota(userId, {
    limit: PAGE_CREATE_LIMIT,
    windowMs: PAGE_CREATE_WINDOW_MS,
  });
  if (rateLimit.allowed) return true;

  res.status(429).json({
    error: {
      code: "PAGE_CREATE_RATE_LIMITED",
      message: "Page creation rate limit reached. Try again shortly.",
...
export async function updateLock(
  ...
  try {
    ...
    const userId = (req as any).user?.userId;
    const { locked, password } = req.body as {
      locked: boolean;
      password?: string;
    };
    ...
    if (locked) {
      if (!password && !(page as any).lockPasswordHash)
        return res.status(400).json({
          error: {
            code: "BAD_REQUEST",
            message: "Password required to lock",
          },
        });
      if (password) {
    ...
    await page.save();
    await cachePageRecord(page);
    res.json({
      ok: true,
      page: { _id: String(page._id), locked: (page as any).locked },
    });
  } catch (err) {
    next(err);
  }
...
export async function verifyLock(
  ...
  try {
    ...
    if (!(page as any).locked)
      return res
        .status(400)
        .json({ error: { code: "BAD_REQUEST", message: "Page not locked" } });
    const ok = (page as any).lockPasswordHash
      ? await bcrypt.compare(password, (page as any).lockPasswordHash)
      : false;
    if (!ok)
    ...
    const token = jwt.sign(
      { sub: String(page._id), typ: "unlock" },
...
export async function getPagesMeta(
  ...
  try {
    ...
    const docs = await Page.find({ _id: { $in: ids } })
      .select("_id title slug icon")
      .lean();
    const pages = docs.map((d: any) => ({
      _id: String(d._id),
      title: d.title,
      slug: d.slug || makeSlug(d.title, String(d._id)),
      icon: d.icon || null,
    }));
    res.json({ pages });
  } catch (err) {
...
export async function listSharedByMe(
  ...
  try {
    ...
    const pages = await Page.find({ ownerId: userId })
      .select("_id title slug isPublic linkEditEnabled collaborators updatedAt")
      .lean();
    const shared = pages.filter((p: any) => {
      const collabEditors =
        Array.isArray(p.collaborators) &&
        p.collaborators.some((c: any) => c.role === "editor");
  ...
  try {
    ...
    if (String(page.ownerId) !== String(userId))
      return res
        .status(403)
        .json({ error: { code: "FORBIDDEN", message: "Forbidden" } });
    const owner = await User.findById(page.ownerId)
      .select("_id email name avatarUrl")
      .lean();
    // Merge legacy sharedWith (editor) and new collaborators with roles
    const collabMap: Record<
      string,
      { _id: string; email: string; name?: string; role: "viewer" | "editor" }
    > = {};
    const collabUsers = Array.isArray(page.collaborators)
      ? page.collaborators
      : [];
    for (const c of collabUsers) {
      const u = await User.findById(c.userId).select("_id email name").lean();
      if (u)
        collabMap[String(u._id)] = {
          _id: String(u._id),
...
export async function inviteCollaborator(
  ...
  try {
    ...
    if (!email)
      return res
        .status(400)
        .json({ error: { code: "BAD_REQUEST", message: "email required" } });
    const page = await Page.findById(id);
    if (!page)
      return res
        .status(404)
    ...
    const existingLegacy =
      Array.isArray(page.sharedWith) &&
      (page.sharedWith as any[]).some((u: any) => String(u) === uid);
    const existingCollab =
      Array.isArray((page as any).collaborators) &&
      (page as any).collaborators.some((c: any) => String(c.userId) === uid);
    if (!existingLegacy && !existingCollab) {
      (page as any).collaborators = Array.isArray((page as any).collaborators)
        ? (page as any).collaborators
        : [];
...
export async function removeCollaborator(
  ...
  try {
    ...
    page.sharedWith = (page.sharedWith || []).filter(
      (u: any) => String(u) !== String(targetUserId),
    ) as any;
    (page as any).collaborators = Array.isArray((page as any).collaborators)
      ? (page as any).collaborators.filter(
          (c: any) => String(c.userId) !== String(targetUserId),
        )
      : [];
    await page.save();
    await cachePageRecord(page);
...
export async function updateCollaboratorRole(
  ...
  try {
    ...
    if (!["viewer", "editor"].includes(role))
      return res
        .status(400)
        .json({ error: { code: "BAD_REQUEST", message: "Invalid role" } });
    const page = await Page.findById(id);
    if (!page)
      return res
        .status(404)
    ...
    (page as any).collaborators = Array.isArray((page as any).collaborators)
      ? (page as any).collaborators
      : [];
    const idx = (page as any).collaborators.findIndex(
      (c: any) => String(c.userId) === String(targetUserId),
    );
    if (idx === -1) {
...
export async function listSharedPages(
  ...
  try {
    ...
    const user = await User.findById(userId)
      .select("sharedLinks sharedBlocked")
      .lean();

    // Start with claimed link shares (these include token)
    const linkShares: any[] = Array.isArray((user as any)?.sharedLinks)
    ...
    const collabPages = await Page.find({
      $or: [{ "collaborators.userId": userId }, { sharedWith: userId }],
    })
    ...
    const ownerIds = Array.from(
      new Set(collabPages.map((p: any) => String(p.ownerId))),
    );
    ...
    for (const o of owners)
      ownerMap.set(String(o._id), {
        name: o.name as any,
        email: o.email as any,
      });
    ...
    try {
      const ids = Array.from(
        new Set(deduped.map((s: any) => String(s.pageId))),
      );
      if (ids.length) {
        ...
        for (const p of pages)
          iconMap.set(String((p as any)._id), (p as any).icon || null);
        deduped = deduped.map((s: any) => ({
          ...s,
          icon: s.icon ?? iconMap.get(String(s.pageId)) ?? null,
        }));
      }
    } catch {}
...
export async function claimLink(
  ...
  try {
    ...
    if (!pageId || !token)
      return res.status(400).json({
        error: { code: "BAD_REQUEST", message: "pageId and token required" },
      });
    ...
    const blocked =
      Array.isArray((user as any).sharedBlocked) &&
      (user as any).sharedBlocked.some(
        (b: any) => String(b.pageId) === String(page._id),
      );
    if (blocked)
      return res.status(403).json({
        error: { code: "FORBIDDEN", message: "Shared page cleared by user" },
      });
    const slug = page.slug || makeSlug(page.title, page._id.toString());
    const arr: any[] = Array.isArray((user as any).sharedLinks)
      ? (user as any).sharedLinks
      : [];
    const exists = arr.some(
      (e: any) => String(e.pageId) === String(page._id) && e.token === token,
    );
    if (!exists) {
...
export async function createPage(
  ...
  try {
    ...
    const allowed = await enforcePageCreateRateLimit(res, userId);
    if (!allowed) return;
...
export async function listMyPages(
  ...
  try {
    ...
    console.log(
      "Pages from cache:",
      pages ? `${(pages as any[]).length} found` : "not found",
    );

    if (!pages) {
      // Cache miss - fetch from database
      console.log("Fetching pages from database...");
      const dbPages = await Page.find({ ownerId: userId })
        .sort({ createdAt: -1 })
        .lean();
...
export async function getPage(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    // Try to get page content from cache first (key: pageId)
    let page: any = null;
    const cachedPage = id ? await RedisService.getPageContent(id) : null;

    if (cachedPage) {
      page = toPageCachePayload(cachedPage);
    }
    ...
    if (!page) {
      ...
      const token =
        typeof req.query.t === "string" ? String(req.query.t) : undefined;
      ...
      if ((page as any).locked) {
        ...
        if (isOwner) {
          ...
          const unlock =
            (req.headers["x-page-unlock"] as string | undefined) || undefined;
          if (!unlock) {
            return res.status(401).json({
              error: {
                code: "LOCKED",
      ...
      // Cache normalized page payload by page id
      const pagePayload = await cachePageRecord(page);
      if (id && pagePayload) page = pagePayload;
    } else {
      ...
      if ((page as any).locked) {
        const isOwner =
          userId && String((page as any).ownerId) === String(userId);
        if (!isOwner) {
          const unlock =
            (req.headers["x-page-unlock"] as string | undefined) || undefined;
          if (!unlock) {
    ...
    if (userId) {
      await RedisService.cacheLastPage(userId, {
        id: String(page._id),
        title: page.title,
        slug: makeSlug(page.title, String(page._id)),
        updatedAt: page.updatedAt,
      });
    }

    const slug = makeSlug(page.title, String(page._id));
    res.json({ page, slug });
  } catch (err) {
    next(err);
  }
  ...
  try {
    ...
    if (!editable)
      return res
        .status(403)
        .json({ error: { code: "FORBIDDEN", message: "Forbidden" } });
    const { title, content, coverImageUrl, coverPosition, icon } =
      req.body as any;
    ...
    // Refresh page cache after changes
    const pagePayload = toPageCachePayload(page);
    if (id) {
      if (prevSlug && prevSlug !== page.slug) {
        await RedisService.invalidatePageSlug(prevSlug);
      }
      if (pagePayload) await RedisService.cachePage(pagePayload);
      else await RedisService.invalidatePageCache(id);
    }
    try {
      const changed =
        (title !== undefined && title !== prevTitle) ||
        (icon !== undefined && icon !== prevIcon);
      if (changed) {
  ...
  try {
    ...
    if (!parent)
      return res.status(404).json({
        error: { code: "NOT_FOUND", message: "Parent page not found" },
      });
    ...
    const tempId = new Date().getTime().toString();
    const slug = makeSlug(pageTitle, tempId);
    const ancestors = Array.isArray((parent as any).ancestors)
      ? [...(parent as any).ancestors, parent._id]
      : [parent._id];
    const sub = await Page.create({
  ...
  try {
    ...
    if (page.ownerId.toString() !== userId)
      return res
        .status(403)
        .json({ error: { code: "FORBIDDEN", message: "Forbidden" } });
    const { isPublic, linkEditEnabled, linkExpiresAt, rotateToken } =
      req.body as {
        isPublic: boolean;
    ...
    if (typeof linkEditEnabled === "boolean")
      page.linkEditEnabled = linkEditEnabled;
    if (linkExpiresAt !== undefined)
      (page as any).linkExpiresAt = linkExpiresAt
    ...
    res.json({
      page: { ...page.toObject(), shareToken: page.shareToken },
      slug,
    });
  } catch (err) {
    next(err);
  ...
  try {
    ...
    if (page.ownerId.toString() !== userId)
      return res
        .status(403)
        .json({ error: { code: "FORBIDDEN", message: "Forbidden" } });
    const { userId: targetUserId, action } = req.body as {
      userId: string;
      action: "add" | "remove";
    };
    const idStr = targetUserId;
    if (action === "add") {
      if (!page.sharedWith.some((id) => id.toString() === idStr))
        page.sharedWith.push(idStr as any);
    } else {
...
export async function deletePage(
  ...
  try {
    ...
    try {
      const ancestors = Array.isArray((page as any).ancestors)
        ? (page as any).ancestors.map((a: any) => String(a))
        : [];
      if (ancestors.length)
        emitSubpageDeletedToAncestors(ancestors, { pageId: String(page._id) });
    } catch {}
    // Collect descendants for cache invalidation, then cascade delete
    const toDelete = await Page.find({ $or: [{ _id: id }, { ancestors: id }] })
      .select("_id slug")
      .lean();
    await Page.deleteMany({ $or: [{ _id: id }, { ancestors: id }] });
    for (const p of toDelete) {
      await invalidatePageRecord(String((p as any)._id), (p as any).slug);
    }
    res.status(204).send();
  } catch (err) {
    next(err);
...
export async function getPageBySlug(
  ...
  try {
    ...
    let page: any = null;
    const cachedPageId = await RedisService.getPageIdBySlug(slug);
    if (cachedPageId) {
      page = await RedisService.getPageContent(String(cachedPageId));
    }

    // Try to find page by slug in database if cache misses
    if (!page) {
      ...
      try {
        ...
        if (ancestorIds.length) {
          const ancestors = await Page.find({ _id: { $in: ancestorIds } })
            .select(
              "_id isPublic shareToken linkExpiresAt linkEditEnabled ownerId",
            )
            .lean();
          const ancestorWithValidToken = ancestors.find(
            (ap: any) => !!ap?.isPublic && isTokenValid(ap, token),
          );
    ...
    if (userId) {
      try {
        ...
        const blocked =
          Array.isArray((u as any)?.sharedBlocked) &&
          (u as any).sharedBlocked.some(
            (b: any) => String(b.pageId) === String(page._id),
          );
        const isCollab =
          Array.isArray((page as any)?.collaborators) &&
          (page as any).collaborators.some(
            (c: any) => String(c.userId) === String(userId),
          );
        if (blocked && !owner && !isCollab) {
          readable = false;
          editable = false;
        }
      } catch {}
    ...
    const pagePayload = await cachePageRecord(page);
    if (pagePayload) page = pagePayload;
...
export async function starPage(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    ...
    if (!page)
      return res
        .status(404)
        .json({ error: { code: "NOT_FOUND", message: "Page not found" } });
    if (!page.favoritedBy.some((u) => u.toString() === userId))
      page.favoritedBy.push(userId as any);
    await page.save();
    await cachePageRecord(page);
    res.status(204).send();
  } catch (err) {
...
export async function unstarPage(
  req: Request,
  res: Response,
  next: NextFunction,
) {
