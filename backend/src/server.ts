...
async function main() {
  ...
  type SavedState = { title?: string; content?: any; coverImageUrl?: string | null; coverPosition?: number; icon?: string | null }
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('page.join', async ({ pageId, token }: { pageId: string; token?: string | null }) => {
      try {
    ...
    socket.on('page.edit', async ({ pageId, title, content, coverImageUrl, coverPosition, icon }: { pageId: string; title?: string; content?: any; coverImageUrl?: string | null; coverPosition?: number; icon?: string | null }) => {
      if (DBG) console.log('page.edit', { pageId, hasTitle: title !== undefined, hasContent: content !== undefined })
      // rate limit
      const now = Date.now()
      ...
      saveTimers.set(pageId, setTimeout(async () => {
        try {
          ...
          if ((state.title !== undefined) || (state.icon !== undefined)) {
            ...
            if (targets.length) emitSubpageMetaToAncestors(targets, { pageId: String(page._id), ...(state.title !== undefined ? { title: state.title } : {}), ...(state.icon !== undefined ? { icon: state.icon } : {}) })
          }
