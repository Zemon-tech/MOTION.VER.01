...
export function PageRoute() {
  ...
  const [ancestors, setAncestors] = useState<
    Array<{ _id: string; title: string; slug: string; icon?: string | null }>
  >([]);
  ...
  function bumpSavingIndicator() {
    ...
    if (savingSequenceTimer.current)
      window.clearTimeout(savingSequenceTimer.current);
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        if (dotIdx > 0 && dotIdx < slug.length - 1)
          token = slug.slice(dotIdx + 1);
        ...
        try {
          cachedUnlock =
            sessionStorage.getItem(`unlock:${slug}`) ||
            sessionStorage.getItem(`unlock-base:${baseSlug}`);
        } catch {}
        ...
        setCoverPosition(
          typeof res.page?.coverPosition === "number"
            ? res.page.coverPosition
            : 50,
        );
        ...
        try {
          ...
          const currentUserId = currentUserRaw
            ? JSON.parse(currentUserRaw)?.id
            : null;
          const favoritedBy = Array.isArray(res.page?.favoritedBy)
            ? res.page.favoritedBy.map((u: any) => String(u))
            : [];
          setIsStarred(
            !!currentUserId && favoritedBy.includes(String(currentUserId)),
          );
        } catch {
          setIsStarred(false);
        }
        ...
        if (id && slug) {
          ...
          try {
            ...
            const arr: Array<{ slug: string; ts: number }> = raw
              ? JSON.parse(raw)
              : [];
            const now = Date.now();
  ...
  useEffect(() => {
    ...
    socket.on("connect_error", (err) => {
      ...
      if (err.message?.toLowerCase().includes("unauthorized")) {
        fetch(
          `${(import.meta as any).env?.VITE_API_BASE || "http://localhost:4000/api"}/auth/refresh`,
          {
            method: "POST",
            credentials: "include",
          },
    ...
    const onUpdated = (payload: any) => {
      ...
      if (payload.coverPosition !== undefined)
        setCoverPosition(payload.coverPosition);
    ...
    const onSubpageMeta = (payload: {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("subpage-meta", { detail: payload }),
        );
      } catch {}
    };
    ...
    const onSubpageDeleted = (payload: { pageId: string }) => {
      try {
        window.dispatchEvent(
          new CustomEvent("subpage-deleted", { detail: payload }),
        );
      } catch {}
    };
  ...
  useEffect(() => {
    async function claim() {
      ...
      try {
        ...
        try {
          window.dispatchEvent(new Event("shared-links-updated"));
        } catch {}
      } catch {}
    }
    claim();
    ...
    <Layout
      header={
        <Header
          ...
          ancestors={ancestors}
          locked={locked}
          onLockChanged={setLocked}
          isStarred={isStarred}
          onStarChanged={setIsStarred}
        />
      }
      sidebar={<AppSidebar />}
    >
      <div className="h-full">
        {showShareToast ? (
          <div
            className="fixed bottom-4 right-4 z-50 rounded-md border text-sm px-3 py-2 shadow
            bg-amber-50 text-amber-900 border-amber-200
            dark:bg-amber-900/80 dark:text-amber-50 dark:border-amber-800"
          >
            You are editing via a share link. Your changes will be saved to this
            page.
          </div>
        ) : null}
        <Editor
          ...
          onCreateSubpage={async (t: string) => {
            ...
            const res = await api<{ page: any; slug: string }>(
          ...
          onCoverImageUrlChange={(url) => {
            ...
            saveTimer.current = window.setTimeout(() => {
              bumpSavingIndicator();
              socket.emit("page.edit", {
                pageId,
                coverImageUrl: url === null ? null : url,
              });
            }, 400);
          }}
            ...
            saveTimer.current = window.setTimeout(async () => {
              try {
                ...
                if (newSlug && newSlug !== currentSlug) {
                  ...
                  if (slugNavTimer.current)
                    window.clearTimeout(slugNavTimer.current);
        ...
        />
        {gateOpen ? <PasswordGate onSubmit={handleGateSubmit} /> : null}
      </div>
    </Layout>
  );
}
