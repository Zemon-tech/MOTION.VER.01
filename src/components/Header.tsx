...
export function Header({ title, savingState = 'idle', pageId, slug, isPublic = false, onPrivacyChanged, ancestors = [], locked = false, onLockChanged }: HeaderProps) {
  ...
  async function lockPage() {
    ...
    if (!pwd) return
    try {
      await api(`/pages/${pageId}/lock`, { method: 'PATCH', body: JSON.stringify({ locked: true, password: pwd }) })
      onLockChanged?.(true)
    } catch {}
  }
  async function unlockPage() {
    if (!pageId) return
    try {
      await api(`/pages/${pageId}/lock`, { method: 'PATCH', body: JSON.stringify({ locked: false }) })
      onLockChanged?.(false)
    } catch {}
  }
  return (
    <div className="w-full flex items-center justify-between gap-2">
      <div className={`flex items-center gap-2 ${state !== 'collapsed' ? 'pl-3 sm:pl-4' : ''}`}>
            ...
            <nav className="flex items-center gap-1">
              {ancestors.map((a) => (
                <span key={a._id} className="inline-flex items-center gap-1">
                  <a href={`/${a.slug}`} className="hover:underline">{a.title || 'Untitled'}</a>
        ...
        <span className="text-xs text-muted-foreground inline-flex items-center gap-1"><Lock className="size-3" /> {isPublic ? 'Public' : 'Private'}</span>
        {savingState !== 'idle' ? (
      ...
      <div className="flex items-center gap-1">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            {locked ? (
              <DropdownMenuItem onClick={unlockPage}>Unlock page</DropdownMenuItem>
            ) : (
