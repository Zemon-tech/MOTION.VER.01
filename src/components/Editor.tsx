...
import { NotionTable, NotionTableRow, NotionTableCell, NotionTableHeader } from '@/components/tiptap-node/table-node/table-node-extension'
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="w-[90vw] sm:max-w-lg" aria-describedby={undefined} showCloseButton={false}>
          <DialogHeader>
            ...
            {tab === 'emoji' ? (
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-end gap-2">
                  <button className="px-3 py-1 text-sm border rounded" onClick={() => setOpen(false)}>Cancel</button>
                  <button className="px-3 py-1 text-sm border rounded bg-foreground text-background" onClick={save}>Save</button>
                </div>
              </div>
            )}
          </div>
  ...
  return (
    <>
      <CoverPickerDialog
        ...
        onSelect={(u) => { onSave(u); setOpen(false) }}
      />
      <button
        className={`inline-flex items-center gap-2 px-2 py-1 rounded-md text-sm text-muted-foreground hover:text-foreground hover:bg-muted/30 transition-colors ${className || ''}`}
        onClick={openDialog}
...
export function Editor({ title, onTitleChange, initialContentJSON, onContentChange, readOnly = false, coverImageUrl, coverPosition = 50, onCoverImageUrlChange, onCoverPositionChange, icon, onIconChange, persistKey, onCreateSubpage }: EditorProps) {
        ...
        addKeyboardShortcuts() {
          return {
            Enter: ({ editor }) => {
              ...
              if (isAtEndOfNode) {
                ...
                return editor
                  .chain()
                  .focus()
                  .insertContentAt(safePos, { type: 'paragraph' }, { updateSelection: true })
                  .run()
              }
            ...
            Backspace: ({ editor }) => {
              ...
              if (isAtStartOfNode && parent.isTextblock && parent.type.name !== 'paragraph') {
                return editor.chain().focus().setParagraph().run()
              }
  ...
  const [selPos, setSelPos] = React.useState<{ x: number; yTop: number; yBottom: number; visible: boolean } | null>(null)
  ...
  useEffect(() => {
    ...
    const fetchMeta = async () => {
      try {
        ...
        const res = await api<{ pages: Array<{ _id: string; title: string; slug: string; icon: string | null }> }>(`/pages/meta?ids=${qs}`)
        ...
        for (const p of pages) {
          try {
            ...
            window.dispatchEvent(new CustomEvent('subpage-meta', { detail: { pageId: p._id, title: p.title, icon: p.icon, slug } }))
          } catch {}
        }
      } catch {}
    }
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('subpage-deleted' as any, handler as any)
  }, [editor])
  ...
  function handleDragEnd(event: any) {
    ...
    // Validate indices
    if (from < 0 || from >= doc.childCount || to < 0 || to >= doc.childCount) return
    ...
    if (from < 0 || from >= content.length || to < 0 || to >= content.length) return
    ...
    const targetPos = Math.min(nextDoc.content.size - 1, Math.max(1, posStart + 1))
  ...
  useEffect(() => {
    ...
    const restore = async () => {
      ...
      try {
        ...
        if (typeof y === 'number') sc.scrollTo({ top: y, behavior: 'instant' as ScrollBehavior })
      } catch {}
    }
    ...
    const onScroll = () => {
      ...
      if (t) window.clearTimeout(t)
      t = window.setTimeout(() => {
        try { localStorage.setItem(key, JSON.stringify({ y: sc.scrollTop })) } catch {}
      }, 150)
    }
  ...
  function deleteTopLevelBlock(index: number) {
    ...
    editor
      .chain()
      .focus()
      .deleteRange({ from: posStart, to: posEnd })
      .run()
  }
            ...
            {menuOpen && (
              ...
              >
                <button
                  ...
                  onClick={() => { deleteTopLevelBlock(index); setMenuOpen(false) }}
                >
                  Delete block
                </button>
              </div>
  ...
  function TableOverlay() {
    ...
    React.useEffect(() => {
      ...
      const updateFromSelection = () => {
        ...
        if (view.dom instanceof HTMLElement) {
          ...
          const fromEl = (anchor instanceof HTMLElement ? anchor : (anchor as any)?.parentElement) as HTMLElement | null
    ...
    const startRowResize = (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      const rowEl = (anchorNode as HTMLElement)?.closest?.('tr') as HTMLTableRowElement | null
      if (!rowEl) return
    ...
    return (
      <>
        ...
        <div
          ...
          style={{ top: rect.top + rect.height - 1, left: rect.left, width: rect.width, height: 28 }}
        >
          <div className="pointer-events-auto flex items-center gap-2" onMouseDown={startRowResize} title="Drag to resize row height">
            <button
              type="button"
              onClick={addRow}
        ...
        <div
          className="absolute z-20 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity"
          style={{ top: rect.top, left: rect.left + rect.width - 1, width: 28, height: rect.height }}
        >
    ...
    <div className="h-full">
      <div className="h-full px-0 py-0">
        <div className="h-full overflow-auto p-0" ref={scrollRef}>
          ...
          {icon ? (
            <IconDisplay icon={icon} hasCover={!!coverImageUrl} />
          ) : null}
          <div className={`px-4 ${coverImageUrl ? 'pt-4' : icon ? 'pt-3' : 'pt-24'}`}>
            <div className="mx-auto w-full max-w-3xl">
              <div className="mb-6 relative group">
                {!readOnly ? (
                  <div className={`flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity z-10 ${coverImageUrl || icon ? 'mt-2' : ''}`}>
                    <IconTrigger
                ...
                {readOnly ? (
                  <h1 className="w-full bg-transparent text-5xl font-semibold break-words">
                    {title && title.trim().length > 0 ? (
                      ...
                      <span className="text-muted-foreground/60 italic select-none">New page</span>
                    )}
                  </h1>
                ) : (
                  <input
                  ...
                  >
                    <SortableContext items={blockIds} strategy={verticalListSortingStrategy}>
                      <div className="relative" id="tiptap-editor-wrapper">
                        ...
                        {editor && !readOnly && selPos?.visible ? (
                          <div
                            ...
                            style={{
                              ...
                              top: (selPos.yTop > 56 ? selPos.yTop : selPos.yBottom),
                              transform: selPos.yTop > 56 ? 'translate(-50%, calc(-100% - 8px))' : 'translate(-50%, 8px)'
                            }}
                          >
                        ...
                        <SlashMenu editor={editor} onCreateSubpage={onCreateSubpage} />
