...
export function CommandPalette() {
  ...
  const fetchResults = React.useCallback(async (q: string) => {
    ...
    try {
      const [pageData, googleData] = await Promise.all([
        ...
        api(`/google/search?q=${encodeURIComponent(q)}`).catch(() => ({
          results: [],
        })),
      ]);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent
        className="p-0 overflow-hidden border-none shadow-2xl max-w-2xl bg-transparent top-[28%]"
      ...
      >
        <Command className="rounded-2xl border border-border/60 bg-popover/95 backdrop-blur-xl text-popover-foreground shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] transition-all duration-200 ring-1 ring-black/5 dark:ring-white/5">
          <div
            className="flex items-center border-b border-border/40 px-5 py-4"
            cmdk-input-wrapper=""
          >
            ...
            />
            {loading ? (
              <Loader2 className="ml-3 h-5 w-5 animate-spin text-primary" />
            ) : (
              <div className="text-[10px] font-black bg-muted/60 px-1.5 py-0.5 rounded border border-border/50 opacity-40">
                ESC
              </div>
            )}
          </div>
          ...
          <Command.List className="max-h-[450px] overflow-y-auto overflow-x-hidden p-3 custom-scrollbar">
              ...
              <Command.Empty className="py-12 text-center">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-sm font-medium text-muted-foreground">
                    ...
                    <span className="text-foreground font-bold underline underline-offset-4 decoration-primary/40">
                      "{query}"
                    </span>
                  </p>
                </div>
            ...
            {!query && recent.length > 0 && (
              <Command.Group
                heading={
                  <div className="flex items-center gap-2 px-2 py-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">
                ...
                {recent.map((item: any) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col min-w-0">
                      ...
                      <span className="text-[10px] uppercase font-black text-muted-foreground/50 tracking-wider">
                        Visited recently
                      </span>
                    </div>
                  </Command.Item>
            ...
            {!query && allPages.length > 0 && (
              <Command.Group
                heading={
                  <div className="flex items-center gap-2 px-2 py-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">
                    <FileText className="size-3" /> All Active Pages
                  </div>
                }
              >
            ...
            {!query && (
              ...
              >
                <Command.Item
                  ...
                  className="flex cursor-pointer select-none items-center rounded-xl p-3 text-sm outline-none aria-selected:bg-accent/80 mb-1"
                >
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-100 text-blue-600 dark:bg-blue-950/40 dark:text-blue-400 mr-4">
                ...
                <Command.Item
                  ...
                  className="flex cursor-pointer select-none items-center rounded-xl p-3 text-sm outline-none aria-selected:bg-accent/80 mb-1"
                >
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-emerald-100 text-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-400 mr-4">
            ...
            {results.length > 0 && (
              <Command.Group
                heading={
                  <div className="flex items-center gap-2 px-2 py-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">
                    <Search className="size-3" /> Search Results
                  </div>
                }
              >
                {results.map((item: any, i: number) => (
                  <Command.Item
                    ...
                    onSelect={() => {
                      ...
                      else if (item.type === "calendar" && item.htmlLink)
                  ...
                  >
                    ...
                    <div className="flex flex-col min-w-0 flex-1">
                      <span className="font-bold truncate text-base leading-tight">
